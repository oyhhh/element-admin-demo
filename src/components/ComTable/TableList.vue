<template>
  <div class="table-form-container_content">
    ...
    <div class="table-header" v-if="!isLayout && search && search.length > 0">
      <el-form :size="size ? size : 'small'" :rules="rules" :model="formSearch" :inline="true" ref="formSearch"
        label-position="right" v-bind="{ ...(options && options.formProps) }">
        <el-form-item :key="index" :prop="item.value" class="table-header-item"
          :label="item.label ? item.label + '：' : ''" v-bind="item.labelProps" v-for="(item, index) in search">
          ...
          <el-input v-else clearable v-bind="item.props" :type="item.inputType || 'text'" v-model="formSearch[item.value]"
            :placeholder="`请输入${item.placeholder || item.label}`" :maxlength="item.maxlength"
            @keyup.enter.native="handleSearch" :oninput="handleChangeInput(item)" />
        </el-form-item>
        <el-form-item>
          <el-button :size="size ? size : 'small'" type="primary" icon="el-icon-search"
            @click="handleSearch">查询</el-button>
          <el-button plain :size="size ? size : 'small'" icon="el-icon-refresh-right"
            @click="handleReset('formSearch')">重置</el-button>
        </el-form-item>
      </el-form>
    </div>

    <el-row :gutter="20" v-if="isLayout && search && search.length > 0">
      <el-form :size="size ? size : 'small'" :rules="rules" :model="formSearch" ref="formSearch"
        v-bind="{ 'label-width': '130px', ...(options && options.formProps) }">
        <el-col :lg="12" :xl="8" :key="index" v-for="(item, index) in search">
          <el-form-item :label="item.label + '：'" :prop="item.value" v-bind="item.labelProps">
            ...
            <el-date-picker clearable style="width: 100%" placeholder="选择日期" value-format="yyyy-MM-dd"
              v-bind="item.props || { type: 'date' }" v-else-if="item.type === 'picker'"
              v-model="formSearch[item.value]" />
            <el-cascader clearable style="width: 100%" v-bind="item.props" :options="item.options"
              v-model="formSearch[item.value]" v-else-if="item.type === 'cascader'"
              :placeholder="`请输入${item.placeholder || item.label}`"></el-cascader>
            <el-row type="flex" :gutter="5" v-else-if="item.type === 'datePicker'">
              <el-col :span="12">
                <el-date-picker clearable style="width: 100%" :editable="false" placeholder="选择开始日期"
                  value-format="yyyy-MM-dd" v-bind="item.props || { type: 'date' }" v-model="formSearch[item.startDate]"
                  :picker-options="startPickerOptions(formSearch[item.endDate])">
                </el-date-picker>
              </el-col>
              <el-col :span="12">
                <el-date-picker clearable :editable="false" style="width: 100%" placeholder="选择结束日期"
                  value-format="yyyy-MM-dd" v-bind="item.props || { type: 'date' }" v-model="formSearch[item.endDate]"
        ...
        <el-col :lg="12" :xl="8">
          <el-form-item label-width="0">
            ...
            <el-button icon="el-icon-refresh-left" @click="handleReset('formSearch')">重置</el-button>
          </el-form-item>
        </el-col>
      </el-form>
    </el-row>
    ...
    <div v-if="extendButton && extendButton.length > 0" class="btn-operates">
      <a :key="index" :href="item.href || null" v-for="(item, index) in extendButton" @click="item.method()">
        <el-button :size="size ? size : 'small'" :type="item.type" :style="{ ...item.style }">
          {{ item.title }}
        </el-button>
      </a>
    </div>
    ...
    <!-- table表格 -->
    <el-table :size="size ? size : 'small'" :border="border" style="width: 100%" :data="dataSource" v-loading="loading"
      v-on="tableEvents" ref="multipleTable" @selection-change="handleSelectionChange" :header-cell-style="
        headerStyle ? headerStyle : { background: '#f5f6f7', color: '#27313d' }" v-bind="{ ...options }">
      <!-- 复选框 -->
      ...
      <template v-for="(column, index) in columns">
        <el-table-column :key="index" v-bind="column.props" :prop="column.prop" :label="column.label"
          :align="column.align ? column.align : 'center'" :width="column.width"
          ...
          <template slot-scope="scope">
            <template v-if="!column.render && !column.slot">
              <template v-if="column.formatter">
                <span v-html="column.formatter(scope.row, column, scope.$index)"
                  @click="column.click && column.click(scope.row, scope.$index)"></span>
              </template>
              <template v-else-if="column.newjump">
                <router-link class="newjump" type="primary" :underline="false"
                  v-bind="{ target: '_blank', ...column.target }" :to="column.newjump(scope.row, column, scope.$index)">{{
            ...
            <template v-else-if="column.slot">
              <slot :column="column" :data="scope.row" :index="index" :name="column.prop" />
            </template>
            <template v-else>
          ...
          <div class="operate-group">
            <template v-for="(btn, key) in operates">
              ...
              ">
                ...
                <template v-if="!btn.render">
                  <el-button :size="btn.size || 'small'" :type="btn.type || `text`" :icon="btn.icon" :plain="btn.plain"
                    :style="
                      btn.setStyle && btn.setStyle(scope.row, scope.$index)
                    " v-bind="btn.props" @click.native.prevent="btn.method(scope.row, scope.$index)" :disabled="
...
">
    ...
    <br />
    <!-- 分页部分 -->
    <el-pagination background v-if="pagination" :total="dataTotal" :page-sizes="[5, 10, 20, 30, 50, 100]"
      :page-size="pagination.limit" @size-change="handleSizeChange" @current-change="handleChangePage"
      :current-page="pagination.page" v-on="options && options.handlePageEvent"
      :class="options && options.pageExtend ? '' : 'pagination'" hide-on-single-page
